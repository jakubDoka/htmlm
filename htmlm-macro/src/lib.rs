...
pub fn html(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let mut st = State {
        final_code: "{ use core::fmt::Write; let mut ___out = String::new(); ".to_string(),
...
impl State {
    ...
    fn expr_then_braces(&mut self, trees: &mut Trees) -> Option<(TokenStream, TokenStream)> {
        let mut body = None;
        let iter = trees
            .by_ref()
            .map_while(|t| {
        ...
        let Some(body) = body else {
            fatal_diag!(Error: "expected '{{' group, got eof");
        };
        Some((iter, body))
    }

    fn if_expr(&mut self, trees: &mut Trees) -> Option<()> {
        let (cond, body) = self.expr_then_braces(trees)?;
    ...
    fn match_expr(&mut self, trees: &mut Trees) -> Option<()> {
        let (expr, body) = self.expr_then_braces(trees)?;
        self.flush_template();
    ...
    fn for_expr(&mut self, trees: &mut Trees) -> Option<()> {
        ...
        let (iter, body) = self.expr_then_braces(trees)?;
